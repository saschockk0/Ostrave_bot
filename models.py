...
# Состояние отмеченного кодируется битовой маской, поэтому держим список коротким
# (до 20 пунктов) и стабильным по порядку — индекс = номер бита.
PACK_ITEMS = [
    "⛺ Палатка и спальник",
    "🧥 Тёплая кофта — у воды ночью свежо",
    "👟 Удобная обувь + 🩴 сланцы",
    "🔦 Фонарик или налобник",
    "🧴 Крем от солнца и средство от комаров",
    ...
    def to_user_summary(self) -> str:
        ...
        if self.extras_note:
            ...
            note = (
                ...
                "катамараны, баня), аренда и снаряжение. Часть оплачивается заранее, "
            ...
            note = (
                ...
                "это трансфер, катамараны и баня; проживание и аренда — отдельно.</i>"
            )
